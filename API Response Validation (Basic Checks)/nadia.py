...
with open('query_Nadia.txt') as f:
    content = f.readlines()
content = [str(x.strip()) for x in content]

url = 'https://abz-nadia.southcentralus.cloudapp.azure.com:443/api/mss/advanced-search/v1/list'
...
nadia = open('Nadia_out.csv', 'w')
with nadia:
    writer = csv.writer(nadia)
    writer.writerows(temp)


nadia_bugs = open('buggy_Nadia_out.csv', 'w')
with nadia_bugs:
    writer = csv.writer(nadia_bugs)
